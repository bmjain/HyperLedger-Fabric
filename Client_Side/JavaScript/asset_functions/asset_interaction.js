...
function createAsset()
{
    ...
    $('#loaderMessages').html('<u>Creating Vehicle</u><br /><br /><span id="messages"></i>waiting...</i></span><br /></div>&nbsp;');
    ...
    xhr.onreadystatechange = function (){
        if(xhr.readyState === 4)
		{
            ...
            else
			{
                ...
                $('#confTxt').html('Created Vehicle: '+ JSON.parse(array[array.length - 1]).v5c);
            }
        }
    };
...
function transferAsset()
{
    ...
    xhr.onprogress = function () {
        ...
        for(let i = 0; i < array.length; i++)
		{
            console.log("the array value is "+ array[i] );
            if(array[i] != '')
			{
                if(typeof JSON.parse(array[i]).error == 'undefined')
				{
    ...
    xhr.onreadystatechange = function (){
		...
		{
            let data = xhr.responseText;
            console.log(" the onreadystatechange data is "+data);
            let array = data.split('&&');
            $('#latestSpan'+(transferIndex)).html('&nbsp;&#10004');
            if(typeof JSON.parse(array[array.length - 1]).error != 'undefined')
			{
...
let bigData;
let increment;
function updateAsset(vin, make, model,year, colour, msrp, v5cID, el)
{
    console.log(msrp);
	/*
	...
	*/
    ...
    bigData = [{'value':vin,'field':'VIN', 'title': 'VIN','v5cID':v5cID},{'value':make,'field':'make', 'title': 'Make','v5cID':v5cID},{'value':model,'field':'model', 'title': 'Model','v5cID':v5cID},{'value':year,'field':'year', 'title': 'Year','v5cID':v5cID},{'value':colour,'field':'colour', 'title': 'Colour','v5cID':v5cID},{'value':msrp,'field':'MSRP', 'title': 'MSRP','v5cID':v5cID}];
    increment=0;
...
function registrationid() {
        ...
        for(let i = 0; i < 3; i++)
            {
            regId += characters.charAt(Math.floor(Math.random() * characters.length));
        }
        regId = numbers.charAt(Math.floor(Math.random() * numbers.length)) + regId;
        //regId = characters.charAt(Math.floor(Math.random() * characters.length)) + regId;
        for(let i = 0; i < 3; i++)
            {
            regId += numbers.charAt(Math.floor(Math.random() * numbers.length));
        }
        return regId;
    }

function computetaxandreg(data,state,msrp)
{
    console.log(JSON.stringify(data));
    console.log(msrp);
    console.log(state);
    if(data.function_name=="private_to_lease_company" || data.function_name=="lease_company_to_private")
    {
    if(state=='California')
    {
        data.statetax=8.5;
        data.sellingprice=1.085 * parseInt(msrp); 
        data.reg=registrationid();
        
    }
    else if(state=='Illinois')
    {
        data.statetax=11.5;
        data.sellingprice=1.115 * parseInt(msrp); 
        data.reg=registrationid();
    }
    else if(state=='Arizona')
    {
        data.statetax=4.0;
        data.sellingprice=1.04 * parseInt(msrp); 
        data.reg=registrationid();
    }
    else
    {
