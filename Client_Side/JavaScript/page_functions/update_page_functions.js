...
function showEditTbl(el)
{
	...
	var year = $(el).siblings('.carYear').html()
	if(year == '&lt;<i>year</i>&gt;')
	{
		year = 'undefined'
	}
	...
	var msrp = $(el).siblings('.carReg').html()
	if(msrp == '&lt;<i>MSRP</i>&gt;')
	{
		msrp = 0;
	}
...
function validate(el)
{
	...
	*/
	...
	if($('#msrp').val().trim() == 0 && $('#hidMsrp').val().trim() != 0)
	{
		$('#errorRw').find('ul').append('<li>MSRP cannot be reset to 0</li>')
		failed = true;
	}
	if($('#make').val().trim() == '')
	{
	...
	{
		$('#errorRw').hide();
		updateAsset($('#vin').val().trim(), $('#make').val().trim(), $('#model').val().trim(),$('#year').val().trim(), $('#colour').val().trim(), $('#msrp').val().trim(), $('#v5cID').val(), el)
	}
	else
	{
		$('#errorRw').show();
