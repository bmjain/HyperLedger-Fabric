	...
	$("#doneRec").on('click', function(){
		$('.recRw').each(function(){
			if($(this).find('.isChk').val() == "true")
			{
				...
				$("#recipientInfo").html("<div><span class='delName'>"+recipients[i].name+"</span> - Identity (<span class='accAddr'>"+recipients[i].identity+"</span>)</div><br /><div style='visibility:hidden'>State:<span class='statename'>"+recipients[i].state+"</span></div>"+getAddress(recipients[i])+"<br /><input id='recAcc' type='hidden' value='" + recipients[i].identity + "'/>")
			}
		})
		$('#chooseRecTbl').hide();
		$('#fade').hide();
