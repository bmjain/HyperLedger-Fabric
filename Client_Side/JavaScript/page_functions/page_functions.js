       ...
       // setCookie();
        ...
        if($('#selVhclsTbl tr').length >= 1 && $('#recipientInfo').html() != '') {
            ...
            $('#selVhclsTbl tr').each(function() {
                ...
                let msrp = $(this).find('.MSRP').val();
                $('#chooseConfHd').html('<span>Transaction Complete</span>');

                $('#confTxt').html('Transaction committed to the blockchain. <br /><br />Manufacturer: '+getCookie('user')+'<br /><br />(Account '+$('.accAddr').html()+')<br /><br />Vehicles: '+$('#selVhclsTbl tr').length);
                ...
                data.value1= $('.statename').html(); //State Name
                data=computetaxandreg(data,data.value1,msrp)
                data.v5cID = v5cID;
