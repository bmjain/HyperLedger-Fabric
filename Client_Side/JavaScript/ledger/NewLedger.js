		...
		success: function(d) {
			...
			for(var i = d.transactions.length-1; i >= 0 ; i--)
			{
				...
				if(obj.payload.indexOf("create_vehicle_log") == -1)
				{
					...
					if(payload.indexOf("update_year") != -1)
					{
						type = "Update";
						function_name = "update_year";
						update_type = "Year";
					}
					if(payload.indexOf("update_msrp") != -1)
					{
						type = "Update";
						function_name = "update_msrp";
						update_type = "MSRP";
					}
					...
					if(type == "Transfer")
					{
						...
						var msrp = get_update("msrp", v5cID);
						var make = get_update("make", v5cID);
						var model = get_update("model", v5cID);
						var year = get_update("year", v5cID);
						var reg = get_update("registration", v5cID);
						var colour = get_update("colour", v5cID);
						var transactiontype="Transfer: "+caller + " -> " + arguments;
						var carDetails = '['+vin+'] '+make+' '+model+', '+msrp+', '+colour
						...
						$('<tr class="retrievedRw" ><td class="transRw">['+v5cID+']</td><td class="transRw">'+vin+'</td><td class="transRw">'+make+'</td><td class="transRw">'+model+'</td><td class="transRw">'+year+'</td><td class="transRw">'+msrp+'</td><td class="transRw">'+transactiontype+'</td><td class="transRw txtRight">'+timestamp+'</td></tr>').insertAfter('#insAft1')
					...
					if(type == "Create")
					{
						...
						var reg = get_update("registration", v5cID);
						var colour = get_update("colour", v5cID);
						var transactiontype="Vehicle Created";
						var modelyear="undefined";
						$('<tr class="retrievedRw" ><td class="transRw">['+v5cID+']</td><td class="transRw">'+vin+'</td><td class="transRw">'+make+'</td><td class="transRw">'+model+'</td><td class="transRw">'+year+'</td><td class="transRw">'+msrp+'</td><td class="transRw">'+transactiontype+'</td><td class="transRw txtRight">'+timestamp+'</td></tr>').insertAfter('#insAft1')
					}
					if(type == "Update")
					{
						console.log(" The Update type is ",update_type)
						console.log(arguments);
						var vin = get_update("vin", v5cID);
						var make = get_update("make", v5cID);
						var model = get_update("model", v5cID);
						var year = get_update("year", v5cID);
						var transactiontype= "<b>"+ update_type + " </b> of the Vehicle Updated";
						var modelyear="undefined";
						var prev = get_update(update_type.toLowerCase(), v5cID);
						if(update_type.toLowerCase()==="vin")
						{
							vin=arguments;
						}
						if(update_type.toLowerCase()==="make")
						{
							make=arguments;
						}
						if(update_type.toLowerCase()==="model")
						{
							model=arguments;
						}
						if(update_type.toLowerCase()==="msrp")
						{
							msrp=arguments;
						}
						if(update_type.toLowerCase()==="year")
						{
							year=arguments;
						}

					//	$('<tr class="retrievedRw " ><td class="smlBrk"></td><td style="width:1%; white-space:nowrap" class="transRw">['+v5cID+'] </td><td class="transRw" style="width:1%; white-space:nowrap"><span class="type" >Update</span><span class="message">: '+caller+'</span></td><td colspan="2" class="transRw">'+update_type+': '+prev+' &rarr; '+arguments+'</td><td class="transRw txtRight">'+timestamp+'</td><td class="smlBrk"></td></tr>').insertAfter('#insAft1')			
					$('<tr class="retrievedRw" ><td class="transRw">['+v5cID+']</td><td class="transRw">'+vin+'</td><td class="transRw">'+make+'</td><td class="transRw">'+model+'</td><td class="transRw">'+year+'</td><td class="transRw">'+msrp+'</td><td class="transRw">'+transactiontype+'</td><td class="transRw txtRight">'+timestamp+'</td></tr>').insertAfter('#insAft1')
					}
				/*	if(type == "Scrap")
					{
						$('<tr class="retrievedRw" ><td class="smlBrk"></td><td style="width:1%; white-space:nowrap" class="transRw">['+v5cID+'] </td><td class="transRw" style="width:1%; white-space:nowrap"><span class="type" >Scrap</span><span class="message">: '+caller+'</span></td><td colspan="2" class="transRw">Scrap V5C</td><td class="transRw txtRight">'+timestamp+'</td><td class="smlBrk"></td></tr>').insertAfter('#insAft1')
					}*/	
					if(failed)
					{
			...
			{
				$('<tr class="retrievedRw" ><td class="smlBrk"></td><td style="width:1%; white-space:nowrap" class="transRw"></td><td class="transRw" style="width:1%; white-space:nowrap"></td><td colspan="2" class="transRw" style="text-align:center">No results found</td><td class="transRw txtRight"></td><td class="smlBrk"></td></tr>').insertAfter('#insAft1')
				$('#filterRw div').hide();
			}
			else
			{
...
function get_update(field, v5cID)
{
	...
	{
	//	console.log(found_cars[v5cID][i].args);
...
function hideType(box, field)
{
	...
	$('.retrievedRw').each(function(){
		if($(this).find('.transRw:eq(6)').html().includes(field))
		{
			$(this).hide();
		}
	})
...
function sortTime(type,initial)
{
	...
	{
		$($(arr[i]).clone()).insertAfter('#insAft1')
	}
...
function changeUser(company, parent, pos)
{
	...
	toggleMenu();
	$('#insAft1').html('<td class="smlBrk"></td><td colspan="5" id="space" style="text-align: center"><img class="loader" src="Images/'+config.participants.users[parent][pos].type.replace(' ', '_')+'/loading.gif" height="50" width="50" alt="loading" text="please wait..." /><br /><br /></td><td class="smlBrk"></td>');
	$('.retrievedRw').remove()
	/*
