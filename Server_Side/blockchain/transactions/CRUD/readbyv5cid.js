...
let result = { 'transactions': [] ,'output':[]};
let height = 1;
let stateHash = '';
let functions = ['create_vehicle','Transfer_vehicle_Ownership','Update_vehicle_details'];
...
function evaluate_transactions(req, res) {
    console.log(req.params.v5cID);
    let validV5cs = '';
    //let user_id = map_ID.user_to_id(req.session.user);
    let user_id = map_ID.user_to_id(req.body.identity);
    if (result.transactions) {
        for (let i = 0; i < result.transactions.length; i++) {
            let transaction = result.transactions[i];
            if(transaction.payload.indexOf("{") !== -1)
            {
                var function_name='';
                for(var j=0;j<functions.length;j++)
                {
                    if(transaction.payload.includes(functions[j]))
                    {
                        function_name=functions[j];
                    }
                }   
            var val = transaction.payload.substring(transaction.payload.indexOf("{"));
            //console.log(JSON.stringify(val));
            var js = JSON.parse(val);
            console.log(js.V5cID);
          //  let v5cID = transaction.payload.match(/[A-Z]{2}[0-9]{7}/g);
            if (req.params.v5cID === js.V5cID) {
                let resultjson={};
                resultjson.object=js;
                resultjson.function=function_name;
                console.log("Inside");    
                result.output.push(resultjson);
                //result.output.push
            }
            else
            {
